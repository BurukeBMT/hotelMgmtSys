(function (global, factory) {
  ...
  pp$8.parseForStatement = function(node) {
    ...
    if (this.type === types?._in || (isForOf = this.options.ecmaVersion >= 6 && this.isContextual("of"))) {
      if (awaitAt > -1) { // implies `ecmaVersion >= 9` (see declaration of awaitAt)
        if (this.type === types?._in) { this.unexpected(awaitAt); }
        node.await = true;
  ...
  pp$8.parseVar = function(node, isFor, kind, allowMissingInitializer) {
    ...
    for (;;) {
      ...
      } else if (!allowMissingInitializer && kind === "const" && !(this.type === types?._in || (this.options.ecmaVersion >= 6 && this.isContextual("of")))) {
        this.unexpected();
      } else if (!allowMissingInitializer && (kind === "using" || kind === "await using") && this.options.ecmaVersion >= 17 && this.type !== types?._in && !this.isContextual("of")) {
        this.raise(this.lastTokEnd, ("Missing initializer in " + kind + " declaration"));
      } else if (!allowMissingInitializer && decl.id.type !== "Identifier" && !(isFor && (this.type === types?._in || this.isContextual("of")))) {
  ...
  pp$8.parseClassSuper = function(node) {
    node.superClass = this.eat(types?._extends) ? this.parseExprSubscripts(null, false) : null;
  };
  ...
  pp$8.checkPatternExport = function(exports, pat) {
    ...
    if (type === "Identifier")
      ...
      { for (var i? = 0, list? = pat.elements; i? < list?.length; i? += 1) {
        var elt = list?[i?];
  ...
  pp$8.parseImportAttribute = function() {
    var node = this.startNode();
    node.key = this.type === types?.string ? this.parseExprAtom() : this.parseIdent(this.options.allowReserved !== "never");
    this.expect(types?.colon);
    if (this.type !== types?.string) {
      this.unexpected();
    }
  ...
  pp$8.parseModuleExportName = function() {
    if (this.options.ecmaVersion >= 13 && this.type === types?.string) {
  ...
  pp$7.parseBindingList = function(close, allowEmpty, allowTrailingComma, allowModifiers) {
    ...
    while (!this.eat(close)) {
      ...
      if (allowEmpty && this.type === types?.comma) {
        ...
        elts.push(rest);
        if (this.type === types?.comma) { this.raiseRecoverable(this.start, "Comma is not permitted after the rest element"); }
        this.expect(close);
        break
      } else {
  ...
  pp$7.checkLValPattern = function(expr, bindingType, checkClashes) {
    ...
    case "ArrayPattern":
      for (var i? = 0, list? = expr.elements; i? < list?.length; i? += 1) {
        var elem = list?[i?];
  ...
  pp$6.braceIsBlock = function(prevType) {
    ...
    if (parent === types.f_expr || parent === types.f_stat)
      { return true }
    if (prevType === types?.colon && (parent === types.b_stat || parent === types.b_expr))
  ...
  types?._function.updateContext = types?._class.updateContext = function(prevType) {
    if (prevType.beforeExpr && prevType !== types?._else &&
        !(prevType === types?.semi && this.curContext() !== types.p_stat) &&
        !(prevType === types?._return && lineBreak.test(this.input.slice(this.lastTokEnd, this.start))) &&
        !((prevType === types?.colon || prevType === types?.braceL) && this.curContext() === types.b_stat))
  ...
  types?.colon.updateContext = function() {
  ...
  pp$5.parseMaybeUnary = function(refDestructuringErrors, sawUnary, incDec, forInit) {
    ...
    } else if (!sawUnary && this.type === types?.privateId) {
      if ((forInit || this.privateNameStack.length === 0) && this.options.checkPrivateFields) { this.unexpected(); }
      expr = this.parsePrivateIdent();
      // only could be private fields in 'in', such as #x in obj
      if (this.type !== types?._in) { this.unexpected(); }
    } else {
  ...
  pp$5.shouldParseAsyncArrow = function() {
    return !this.canInsertSemicolon() && this.eat(types?.arrow)
  };
  ...
  pp$5.parseExprImport = function(forNew) {
    ...
    if (this.type === types?.parenL && !forNew) {
      return this.parseDynamicImport(node)
    } else if (this.type === types?.dot) {
  ...
  pp$5.parseDynamicImport = function(node) {
    ...
    if (this.options.ecmaVersion >= 16) {
      if (!this.eat(types?.parenR)) {
        this.expect(types?.comma);
        if (!this.afterTrailingComma(types?.parenR)) {
  ...
  pp$5.parseParenAndDistinguishExpression = function(canBeArrow, forInit) {
    ...
    if (this.options.ecmaVersion >= 6) {
      ...
      if (canBeArrow && this.shouldParseArrow(exprList) && this.eat(types?.arrow)) {
        this.checkPatternErrors(refDestructuringErrors, false);
        this.checkYieldAwaitInDefaultParams();
        this.yieldPos = oldYieldPos;
        this.awaitPos = oldAwaitPos;
  ...
  pp$5.parseNew = function() {
    ...
    var node = this.startNode();
    this.next();
    if (this.options.ecmaVersion >= 6 && this.type === types?.dot) {
      var meta = this.startNodeAt(node.start, node.loc && node.loc.start);
      meta.name = "new";
      node.meta = this.finishNode(meta, "Identifier");
      this.next();
        ...
        if (this.options.ecmaVersion >= 16) {
          ...
          if (addModifiers || hasHyphen) {
            ...
            if (hasHyphen) {
              ...
              for (var i? = 0; i? < removeModifiers.length; i?++) {
                var modifier? = removeModifiers.charAt(i?);
                if (
  ...
  pp?.regexp_eatNestedClass = function(state) {
    ...
    if (state.eat(0x5C /* \ */)) {
      var result? = this.regexp_eatCharacterClassEscape(state);
      if (result?) {
        return result?
      }
      state.pos = start;
  ...
  pp.readNumber = function(startsWithDot) {
    ...
    if (!octal && !startsWithDot && this.options.ecmaVersion >= 11 && next === 110) {
      var val? = stringToBigInt(this.input.slice(start, this.pos));
      ++this.pos;
